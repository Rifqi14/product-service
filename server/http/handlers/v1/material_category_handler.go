...
func NewMaterialCategoryHandler(handler handlers.Handler) hinterface.IMaterialCategoryHandler {
	return &MaterialCategoryHandler{Handler: handler}
}

func (handler MaterialCategoryHandler) Create(ctx *fiber.Ctx) (err error) {
	req := new(request.MaterialCategoryRequest)
...
func (handler MaterialCategoryHandler) List(ctx *fiber.Ctx) (err error) {
	...
	return responses.NewResponse(responses.ResponseSuccess(data, meta, "data material category success fetched")).Send(ctx)
}

func (handler MaterialCategoryHandler) Detail(ctx *fiber.Ctx) (err error) {
...
func (handler MaterialCategoryHandler) Delete(ctx *fiber.Ctx) (err error) {
	categoryId, err := uuid.Parse(ctx.Params("id"))
	if err != nil {
		return responses.NewResponse(responses.ResponseError(nil, nil, http.StatusBadRequest, messages.FailedLoadPayload, err)).Send(ctx)
	}
	...
	return responses.NewResponse(responses.ResponseSuccess(nil, nil, "Success delete material category")).Send(ctx)
}
